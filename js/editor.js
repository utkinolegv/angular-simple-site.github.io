...
$.fn.juirte = function(options){
	...
	var settings = $.extend({
		resizable: false,
		stylesheet: '',
		language: [['html', 'Показать HTML'],['insertimage', 'Добавить картинку'],['inserturl', 'Добавить URL'],['paragraph', 'Параграф'],['address', 'Адрес'],['heading', 'Заголовок'],['bgcolor', 'Цвет фона'],['fontcolor', 'Цвет текста'],['fontsize', 'Размер'],['font', 'Фонт'],['paragraph', 'Параграф'],['link', 'Ссылка'],['removelink', 'Удалить ссулку'],['italic', 'Курсив'],['bold', 'Полужирный'],['underline', 'Подчеркнутый'],['strike', 'Зачеркнутный'],['hr', 'Горизонтальная линия'],['ol', 'Нумерованный список'],['ul', 'Маркированный список'],['center', 'По центру'],['left', 'По левому краю'],['full', 'По ширине'],['right', 'По правому краю'],['indent', 'Увеличить отступ'],['outdent', 'Уменьшить отступ'],['superscript', 'Надстрочный знак'],['subscript', 'Подстрочный знак'],['rm', 'Удалить форматирование']],
		width : "2000px",
		height : "200px",
	...
	return this.each(function(){
       ...
       $this.after(_container); 
		...
		var wysiwyg_menu = $("<div/>",{
			"class" : "ui-widget ui-widget-content ui-widget-header ui-corner-top ui-wysiwyg-menu",
			css : { width : '100%' }
		...
		function fnRunDialog(type){
            ...
            width: 440,
					...
					}).html('<input type="text" style="width:400px;" value="http://"/>');
				break;	
            ...
            width: 440,
						buttons: {
							"Ok": function() {
                if ($(this).find('#link').val()!='http://')
								 fnRunCommand(type, $(this).find('#link').val());
                else
                if ($(this).find('#picfile').val()!=null)
                {
                    $("#spinner").fadeIn("slow");
                    var formData = new FormData();
                    formData.append('file', $(this).find('#picfile')[0].files[0]);
                    $.ajax({
                     url: "picupload.json",
                     type: "POST",
                     data: formData,
					...
					}).html('<p>В виде ссылки:</p><p><input id="link" type="text" style="width:400px;" value="http://"/></p><p>или укажите файл (gif, jpeg, jpg, png) на диске (размер не более 200 кб):</p><p><input type="file" id="picfile"></p>');
				break;
			}
		}
		...
		function fnExecCommand (e) {
          ...
          if ($(_wrapper).find('.ui-wysiwyg-btn').button("option", "disabled"))
           $(_wrapper).find('.ui-wysiwyg-btn').button("option", "disabled", false);
          else
           $(_wrapper).find('.ui-wysiwyg-btn').button("option", "disabled", true);
          $(_wrapper).find('.ui-wysiwyg-btn-html').button("option", "disabled", false);
