...
angular.module('wysiwyg.module', ['colorpicker.module'])
  .directive('wysiwyg', function ($timeout, $http) {
    return {
        template: '<div>' +             
                    '<style>' +
                    ...
                    '</div>' +
                    '<div class="btn-group btn-group-sm wysiwyg-btn-group-margin">' +
                        '<select unselectable="on" tabindex="-1" class="form-control wysiwyg-select" ng-model="picture" ng-options="p.namesize for p in pictures" ng-change="setPicture()">' +
                        '</select>' +
                    '</div>' +
                    '<div class="btn-group btn-group-sm wysiwyg-btn-group-margin">' +
                        '<select unselectable="on" tabindex="-1" class="form-control wysiwyg-select" ng-model="ufile" ng-options="f.namesize for f in ufiles" ng-change="setFile()">' +
                        '</select>' +
                    '</div>' +
        ...
        scope.fonts = [
            ...
            $http.get("backend/getuploaded.php?dir=picupload").then(function (response) {
             scope.pictures = response.data.records;
             scope.pictures.sort();
             scope.pictures.unshift({namesize:'Выбрать картинку'});
             scope.picture = scope.pictures[0];
             //console.log(scope.pictures);
            });

            $http.get("backend/getuploaded.php?dir=filesupload").then(function (response) {
             scope.ufiles = response.data.records;
             scope.ufiles.sort();
             scope.ufiles.unshift({namesize:'Выбрать файл'});
             scope.ufile = scope.ufiles[0];
             //console.log(scope.ufiles);
            });
            ...
            scope.createLink = function(){
                var input = prompt('Введите адрес URL (пример http://www.example.com/page.html)');
                if (input && input !== undefined)
                    scope.format('createlink', input);
            }

            scope.insertImage = function(){
                var input = prompt('Введите URL картинки');
                if (input && input !== undefined)
            ...
            scope.setPicture = function(){
                if (scope.picture.name!='Выбрать картинку') {
                 scope.format('insertimage', 'picupload/' + scope.picture.name)
                }
            }

            scope.setFile = function(){
                if (scope.ufile.name!='Выбрать файл') {
                  var input = prompt('Введите наименование файла');
                  if (input && input !== undefined)
                    scope.format('inserthtml', '<a href="filesupload/' + scope.ufile.name + '">' + input + '</a>');
                }
            }
